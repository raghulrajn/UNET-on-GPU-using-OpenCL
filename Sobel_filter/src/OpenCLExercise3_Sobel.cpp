...
// CPU implementation
//////////////////////////////////////////////////////////////////////////////
int getIndexGlobal(std::size_t countX, int i, int j) { return j * countX + i; }
// Read value from global array a, return 0 if outside image
float getValueGlobal(const std::vector<float>& a, std::size_t countX,
                     std::size_t countY, int i, int j) {
  if (i < 0 || (size_t)i >= countX || j < 0 || (size_t)j >= countY)
    return 0;
  else
    return a[getIndexGlobal(countX, i, j)];
}
void sobelHost(const std::vector<float>& h_input,
               std::vector<float>& h_outputCpu, std::size_t countX,
               std::size_t countY) {
...
cl::Sampler createSobelSampler(cl::Context context) {
    // Create the sampler using C++ OpenCL API
    cl_int err;

    cl::Sampler sampler(
        context,                        // OpenCL context
        CL_TRUE,                         // Normalized coordinates
        CL_ADDRESS_CLAMP_TO_EDGE,        // Addressing mode
        CL_FILTER_LINEAR,                // Filtering mode
        &err                             // Error code
    );

    if (err != CL_SUCCESS) {
        // Handle error if necessary
        std::cerr << "Error creating sampler: " << err << std::endl;
    }
...
int main(int argc, char** argv) {
  std::cout << "------------------------------------------------" << std::endl;
  std::cout << "OpenCL Exercise 3: Sobel" << std::endl;
  std::cout << "------------------------------------------------" << std::endl;
  ...
  extern unsigned char OpenCLExercise3_Sobel_cl[];
  extern unsigned int OpenCLExercise3_Sobel_cl_len;
  cl::Program program(context,
                      std::string((const char*)OpenCLExercise3_Sobel_cl,
                                  OpenCLExercise3_Sobel_cl_len));
  ...
  std::vector<float> d_outputCpu(count);
  std::vector<float> d_outputGpu(count);
  ...
  cl::Image2D inputImage(context,CL_MEM_READ_WRITE| CL_MEM_COPY_HOST_PTR,format,inputWidth, inputHeight,0, h_input.data());
  cl::Image2D outputImage(context,CL_MEM_READ_WRITE,format,inputWidth, inputHeight);
  cl::Sampler sobelSampler = createSobelSampler(context);
  ...
  for (int impl = 1; impl <= 1; impl++) {
    ...
    memset(h_outputGpu.data(), 255, size);
    queue.enqueueWriteImage(inputImage, true, origin, region, inputWidth*sizeof(float), 0, h_input.data());
    cl::Kernel kernel(program, "sobelKernel3");
    kernel.setArg<cl::Image2D>(0, inputImage);
    kernel.setArg<cl::Image2D>(1, outputImage);
    kernel.setArg(2, sobelSampler);  // Sobel sampler

// Define the global size (image dimensions)
cl::NDRange globalSize(inputWidth, inputHeight);

// Run the kernel
queue.enqueueNDRangeKernel(kernel, cl::NDRange(), cl::NDRange(inputWidth, inputHeight),cl::NDRange(wgSizeX, wgSizeY),NULL, &event);
queue.enqueueReadImage(outputImage, CL_TRUE, origin, region, 0, 0, h_outputGpu.data());
queue.finish();

    Core::writeImagePGM(
        "output_sobel_gpu_3.pgm",
        h_outputGpu, countX, countY);
