...
int main(int argc, char** argv) {
  std::cout << "------------------------------------------------" << std::endl;
  std::cout << "OpenCL Exercise 2: Mandelbrot" << std::endl;
  std::cout << "------------------------------------------------" << std::endl;
  ...
  int64_t maxError = 0;  // maximum difference between CPU and GPU solution (to account for rounding errors)
  ...
  // Declare some values
  std::size_t wgSizeX = 4;  // Number of work items per work group in X direction
  ...
  //TODO
  cl::Buffer d_output(context, CL_MEM_READ_WRITE, sizeof(cl_uint)*count);
  ...
  mandelbrotKernel.setArg<std::size_t>(6,countX);
  mandelbrotKernel.setArg<std::size_t>(7,countY);

  queue.enqueueNDRangeKernel(mandelbrotKernel, cl::NDRange(), cl::NDRange(countX, countY),cl::NDRange(wgSizeX, wgSizeY),NULL, &event);
  ...
  std::cout<<"GPU time is "<<timeGpu<<std::endl;
